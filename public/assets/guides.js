  ...
  const guides = [
    ...
    guide("facebook", s, {
      ...
      steps: [
        ...
        `<strong>Create a dedicated app for Facebook</strong>, separate from the Instagram one, logged in as the account that will do the connecting: My Apps → Create app → use case <strong>"Authenticate and request data from users with Facebook Login"</strong> (a Consumer-type app with classic Facebook Login). Avoid "Other → Business" here — Business apps get "Facebook Login for Business", which needs a config ID, a connected business portfolio, and fails with generic errors far more easily.`,
        `Facebook Login → <strong>Settings</strong>: <strong>Client OAuth login</strong> and <strong>Web OAuth login</strong> both ON, and paste the redirect URI above into <strong>Valid OAuth Redirect URIs</strong> (the <em>Redirect URI Validator</em> box on that page tells you if it's accepted).`,
        `App settings → Basic: <strong>App Domains</strong> ${code(base.replace(/^https?:\/\//, ""))}, at the bottom <strong>Add Platform → Website</strong> with Site URL ${code(base + "/")}, plus the <a href="/privacy" target="_blank">/privacy</a> and <a href="/terms" target="_blank">/terms</a> URLs. Save.`,
        webhookStep,
        `Copy the <strong>App ID</strong> and <strong>App Secret</strong> from App settings → Basic into the Railway variables and redeploy. Leave <code>FACEBOOK_CONFIG_ID</code> empty for a Consumer app — it's only for Business-type apps (see below).`,
        `<strong>Keep the app in Development mode.</strong> That's the intended permanent state for posting to your own Pages: the app creator can grant all Pages permissions with no App Review. Going Live requires Business Verification + App Review and breaks login with a generic error until both pass.`,
        ...
        `<strong>Only if you must use a Business-type app</strong> (e.g. reusing the Instagram app): the login product is "Facebook Login for Business" and additionally needs a <em>configuration</em> — Configurations → Create, token type <strong>User access token</strong> (NOT System-user), the three pages permissions — its Configuration ID goes in <code>FACEBOOK_CONFIG_ID</code>, and the app must be connected to a business portfolio (App settings → Advanced).`,
      ],
      gotchas: [
        ...
        `<strong>"Invalid parameter: config_id is required"</strong> → the app is Business-type: either set <code>FACEBOOK_CONFIG_ID</code> (see the last step) or switch to a Consumer-type app.`,
        `<strong>Generic "Sorry, something went wrong" on the login dialog</strong> → in order of likelihood: the app is Live (switch to Development); the configuration's token type is System-user instead of User; the logged-in Facebook account has no role on the app (use the creator account, in incognito); no business portfolio connected (Business apps only). If all four are right, it's Meta being flaky — retry later.`,
