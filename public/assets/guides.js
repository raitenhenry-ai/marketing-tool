  ...
  const guides = [
    ...
    guide("linkedin", s, {
      ...
      steps: [
        ...
        `<strong>To post as a Company Page instead:</strong> there's no separate "business login" on LinkedIn — you always sign in with your personal account, and Pages you admin ride on top. LinkedIn requires a <strong>separate app</strong> for this: create a second app, and in its <strong>Products</strong> tab request ONLY <strong>"Community Management API"</strong> (it must be the sole product — adding others blocks the request). After LinkedIn approves it (form + review, usually days): add the redirect URI above to that app's Auth tab, put ITS Client ID/Secret in the Railway variables, set <code>LINKEDIN_COMPANY_PAGES=true</code>, redeploy, and reconnect — every Page you super-admin becomes its own account, posting as the Page. (Personal-feed posting doesn't exist on that app; disconnect the old personal LinkedIn account.)`,
      ],
